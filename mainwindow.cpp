...
void MainWindow::onContentsChange(int position, int charsRemoved, int charsAdded) {
    ...
    auto list = treeParserSyntaxHighlighter.getHighlightBlocks(ts_tree_root_node(newTree));
    ...
    treeParserSyntaxHighlighter.updateHighlighting(textDocument, list, textEdit->textCursor().position(), tree, newTree, textEdit->textCursor().position() == -1);
    tree = newTree;
