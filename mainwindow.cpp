    ...
    , ui(new Ui::MainWindow)
{
    ...
    stringsFormat.setForeground(c3);
    commentsFormat.setForeground(c4);
    variablesFormat.setForeground(QColor(50, 168, 160));
    typesFormat.setForeground(c2);
    functionsFormat.setForeground(QColor(50, 168, 121));
    extraFormat.setForeground(c1);
    greyFormat.setForeground(c5);
    literalFormat.setForeground(QColor(245, 120, 66));

    treeParserSyntaxHighlighter.setFormats({nothingFormat, stringsFormat, commentsFormat, variablesFormat, typesFormat, functionsFormat, extraFormat, greyFormat, literalFormat});
    ...
    highlighter = new CodeHighlighter(currentLang, c3Format, c4Format, c5Format, codeWizForm, textDocument);
...
void MainWindow::onContentsChange(int position, int charsRemoved, int charsAdded) {
    ...
    tree = ts_parser_parse_string(parser, tree, documentText.constData(), documentText.size());

    qDebug() << "To update tree: " << timer.elapsed();

    auto list = treeParserSyntaxHighlighter.getHighlightBlocks(ts_tree_root_node(tree));

    qDebug() << "To get list: " << timer.elapsed();

    treeParserSyntaxHighlighter.updateHighlighting(textDocument, list);
